...
def test_set_session_env_includes_session_key():
    ...
    # Capture whatever the SESSION_KEY was before this test touches it so
    # we can verify _clear_session_env restores it exactly (not assume "").
    before_value = get_session_env("HERMES_SESSION_KEY")
    ...
    assert get_session_env("HERMES_SESSION_KEY") == "tg:-1001:17585"
    runner._clear_session_env(tokens)
    # _clear_session_env must restore the pre-set value, whatever it was
    assert get_session_env("HERMES_SESSION_KEY") == before_value
