...
# Minimal models.dev response that includes opencode-go so the test is
# network-independent and works on CI without internet access.
_FAKE_MODELS_DEV = {
    "opencode-go": {
        "name": "opencode-go",
...
def test_opencode_go_appears_when_api_key_set(mock_fetch):
...
@patch("agent.models_dev.fetch_models_dev", return_value=_FAKE_MODELS_DEV)
def test_opencode_go_not_appears_when_no_creds(mock_fetch):
